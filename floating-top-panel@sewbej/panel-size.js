...
/* -------------------------------------------------------
   CONFIGURATION (top PANEL)
------------------------------------------------------- */

const TARGET_POS        = Panel.PanelLoc.top;  // this module only affects top panels
...
let settings          = null;
let originalHeightsMap = {};   // panelId -> original "100%" height (top panels only)
...
function enable() {
    ...
    // read original top panel heights and assign them to monitors
    readOriginalHeights();
...
function disable() {
    ...
    // restore original heights of top panels
...
function hookWindow(win) {
    try {
        ...
        if (!win || win._fbpTrackedtop) return;
        win._fbpTrackedtop = true;
...
function updateWindowTracking(win) {
    try {
        ...
        const prev = win._fbpInfotop || { monitor: -1, isMax: false };
        ...
        win._fbpInfotop = { monitor: nowMon, isMax: nowIsMax };
...
function clearWindowTracking(win) {
    try {
        if (!win || !win._fbpInfotop) return;

        const info = win._fbpInfotop;
        if (info.isMax && info.monitor >= 0)
            decMonitor(info.monitor);

        delete win._fbpInfotop;
    } catch (e) {
        // ignore
    }
}
...
function rebuildMaximizedState() {
    ...
    try {
        ...
        ws.list_windows().forEach(win => {
            ...
            win._fbpInfotop = { monitor: mon, isMax };
...
function restoreOriginalHeights() {
    ...
    // restore only top panels we manage
...
function updatePanelHeights() {
    ...
    // adjust only top panels we manage
    for (let id in originalHeightsMap) {
