...
const TARGET_POS        = Panel.PanelLoc.bottom;  // this module only affects bottom panels
...
let settings          = null;
let originalHeightsMap = {};   // panelId -> original "100%" height (bottom panels only)
...
function enable() {
    ...
    // read original bottom panel heights and assign them to monitors
    readOriginalHeights();
...
function disable() {
    ...
    // restore original heights of bottom panels
...
function hookWindow(win) {
    try {
        ...
        if (!win || win._fbpTrackedBottom) return;
        win._fbpTrackedBottom = true;
...
function updateWindowTracking(win) {
    try {
        ...
        const prev = win._fbpInfoBottom || { monitor: -1, isMax: false };
        ...
        win._fbpInfoBottom = { monitor: nowMon, isMax: nowIsMax };
...
function clearWindowTracking(win) {
    try {
        if (!win || !win._fbpInfoBottom) return;

        const info = win._fbpInfoBottom;
        if (info.isMax && info.monitor >= 0)
            decMonitor(info.monitor);

        delete win._fbpInfoBottom;
    } catch (e) {
        // ignore
    }
}
...
function rebuildMaximizedState() {
    ...
    try {
        ...
        ws.list_windows().forEach(win => {
            ...
            win._fbpInfoBottom = { monitor: mon, isMax };
...
function restoreOriginalHeights() {
    ...
    // restore only bottom panels we manage
...
function updatePanelHeights() {
    ...
    // adjust only bottom panels we manage
    for (let id in originalHeightsMap) {
